...
public class PersonControllerTest {
    ...
    void testGivenPersonObject_whenCreatePerson_thenReturnSavedPerson() throws Exception {
        ...
        //When / Act
        ResultActions response = mockMvc.perform(post("/person").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(person)));
        //Then / Assert
    ...
    @DisplayName("Given List Persons when Find All Persons then Return Persons List")
    @Test
    void testGivenListPersons_whenFindAllPersons_thenReturnPersonsList() throws Exception {
        //Given / Arrange
        List<Person> persons = new ArrayList<>();
        persons.add(person);
        persons.add(new Person("Suelen", "Belucio", "Sao Paulo", "Female", "suelen@gmail.com"));
    ...
    @Test
    void testGivenPersonId_whenFindById_thenReturnPersonId() throws Exception {
        //Given / Arrange
        long personId = 1L;
    ...
    @DisplayName("Given Update Person when Update then Return Person Object")
    @Test
    void testGivenUpdatedPerson_whenUpdate_thenReturnPersonObject() throws Exception {
        //Given / Arrange
        long personId = 1L;
        given(service.findById(personId)).willReturn(person);
        given(service.update(any(Person.class))).willAnswer((invocation) -> invocation.getArgument(0));
        //When / Act
        Person updatedPerson = new Person("Suelen", "Belucio", "Sao Paulo", "Female", "suelen@gmail.com");

        ResultActions response = mockMvc.perform(put("/person").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(updatedPerson)));
        //Then / Assert
    ...
    @DisplayName("Given Person Id when Update then Return NotContent")
    @Test
